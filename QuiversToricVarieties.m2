...
--the forbidden sets for Y using the enumeration for Y
   forbiddens := apply(flatten values A1, i -> apply(i, j -> Perm#j));
--the forbidden sets for Y using the enumeration for X
   generalisations := subsets (select(numgens tDiv.source, i -> 
	   tDiv_{i} == 0));        
--subsets of all the contracted rays
   alpha := flatten apply(forbiddens, i -> apply(
	   generalisations, j -> i|j));
--forbidden sets from Y pulled back to X
   beta = unique(beta|alpha);
   L = delete(last L, L);
	    );
 gamma:= forbiddenSets X;
 gamma#1 = unique(gamma#1 | beta);
--combining the pulled back forbidden sets to those for X
 d := #rays X;
 A := fromWDivToCl X;      
